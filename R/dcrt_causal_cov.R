...
#' cramed.res <- dCRT_causal_cov(M_mat=sim_zinb.mat$M_mat,Y=sim_zinb.mat$Y_mat, Exposure=sim_zinb.mat$t,cov=matrix(rnorm(num)),n.perm=10, CI=TRUE)
#' @export
...
dCRT_causal_cov <- function(M_mat, Y, Exposure, cov, FDR = 0.05, n.times=100, prefilter=TRUE,
  ...
  if (prefilter == TRUE){
    set.seed(123)
    cv_lasso <- cv.glmnet(cbind(Exposure,cov,M_mat), Y, alpha = 1, family = model,
  ...
  if(length(selection_set)==0){pvl_lst=NA}
    ...
    if(p==1){
      Cond_M <- Creat_condition_zinb_cov(M_mat, indx=1, Exposure, n.times,cov=cov)
      mean_M <- Cond_M$mean_m
      M_res_ob <- Cond_M$res_m
      data <- data.frame(Y=Y,Exposure=Exposure,cov=cov)
      if (model == 'binomial'){
        eps_res <- Y - 1 / (1 + exp(- predict(lm(Y~Exposure+cov,family = model,data=data))))
      }
      if (model == 'gaussian'){
        eps_res <- Y - predict(lm(Y~Exposure+cov,family = model,data=data))
      }
    ...
    if(p>1){
      ...
      set.seed(123)
      cv_lasso_null <- cv.glmnet(cbind(M_mat,cov), Y, alpha = 1, family = model,
                                 dfmax = as.integer(p / 2))
      ...
      for (j in 1:length(selection_set)){
        ...
        Cond_M <- Creat_condition_zinb_cov(M_mat, indx, Exposure, n.times,cov=cov)
        mean_M <- Cond_M$mean_m
        M_res_ob <- Cond_M$res_m

        ############### Distill Y and Importance ##################
        set.seed(123)
        cv_lasso_null <- cv.glmnet(cbind(M_mat[,-indx],Exposure,cov), Y, alpha = 1, family = model,
                                   dfmax = as.integer(p / 2))
        lamb_null <- cv_lasso_null$lambda.min
  ...
  for(j in 1:length(selection_set)){
    ...
    if(sum(Y_data$Mediator==0)==0){
      b <-  glm.nb(Mediator ~ Exposure+cov, data = Y_data)
  ...
  if(CI==TRUE){
    if(length(mediation_set)==0){
      ...
      for(jk in 1:n.perm){
        ...
        cv_lasso <- cv.glmnet(as.matrix(data.frame(M_mat.perm,t.perm,cov.perm)), Y_mat.perm, alpha = 1, family = 'gaussian',
