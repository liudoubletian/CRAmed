    ...
    if (!all.rarefy | test.omni3) {
        ...
        for (r in 1:n.rarefy) {
            ...
            if (binary) {
                ...
                otu.rarefy = otu.rarefy/max(abs(rowSums(otu.rarefy)))
            }
...
permanovaFL = function(formula, other.surv.resid=NULL, data=.GlobalEnv, tree=NULL, dist.method=c("bray"), dist.list=NULL, 
    ...
    if (!is.null(dist.list)) {
        ...
        if (!is.null(dist)) {
            ...
            scale.otu.table=rep("TRUE", n.dist)
...
permanova.stat = function(x, low, up, resid.dist, ndf, adjust.for.confounders) {
    ...
    for (r in 1:n.rarefy) {
        ...
        for (k in 1:n.var1) {
            ...
            numerator = sum( diag(Hk %*% resid.dist[1,,,k,r]) )   # the other Hk and I_Hcum is not needed
            denominator = sum( diag(I_Hcum %*% resid.dist[1,,,k,r]) )
